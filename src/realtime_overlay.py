...
def draw_overlay(
    ...
    for x1, y1, x2, y2, label, confidence in boxes:
        color = (0, 0, 255) if label in NG_CLASSES else (0, 180, 0)
    ...
    status_color = (0, 0, 255) if status == "NG" else (0, 180, 0) if status == "PASS" else (0, 180, 255)
    ...
    cv2.putText(canvas, status, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (0, 0, 0), thickness + 5, cv2.LINE_AA)
    cv2.putText(canvas, status, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, font_scale, status_color, thickness, cv2.LINE_AA)

    metadata_lines = [
        f"Point: {point_number}",
        f"CSV: {product_info.csv_path.name if product_info.csv_path else '-'}",
        f"SN: {product_info.sn or '-'}",
        f"CutedTable: {product_info.cuted_table or '-'}",
    ...
    for line in metadata_lines:
        cv2.putText(canvas, line, (16, y), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (0, 0, 0), 4, cv2.LINE_AA)
        cv2.putText(canvas, line, (16, y), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (255, 255, 255), 2, cv2.LINE_AA)
        y += 28
    return canvas
